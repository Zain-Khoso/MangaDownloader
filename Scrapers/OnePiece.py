...
def download_OnePiece_Manga():
    ...
    cwURL = "https://onepiecechapters.com/chapters/433/one-piece-chapter-1"
...
def getNextURL(DOM, cwURL):
    ...
    cwURLRegex = re.compile(r"(.*?)(/chapters)(/\d+)(/one-piece-chapter-(\d+))")

    # 2 - Searches the Current-Chapter-URL using Regex and gets the next-Chapter-Name.
    cChpURLSearch = cwURLRegex.search(cwURL)
    nChpName = f"one-piece-chapter-{str(int(cChpURLSearch.group(5)) + 1)}"
...
def downloadChapter(DOM, chNum):
    ...
    pageElems = DOM.select('img[alt~="Piece"]')
    threads = []
