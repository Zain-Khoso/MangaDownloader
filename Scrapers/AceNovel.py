# main.py - Manages all the work needed to download AceNovel manga.
...
def download_AceNovel_Manga():
    # Setting up the rootURL and cwURL for the downloader.
    cwURL = (
        "https://onepiecechapters.com/chapters/44/ace-novel-manga-adaptation-chapter-1"
    )
...
def getNextURL(DOM, cwURL):
    ...
    cwURLRegex = re.compile(
        r"(.*?)(/chapters)(/\d+)(/ace-novel-manga-adaptation-chapter-(\d+))"
    )

    # 2 - Searches the Current-Chapter-URL using Regex and gets the next-Chapter-Name.
    cChpURLSearch = cwURLRegex.search(cwURL)
    nChpName = (
        f"ace-novel-manga-adaptation-chapter-{str(int(cChpURLSearch.group(5)) + 1)}"
    )
...
def downloadChapter(DOM, chNum):
    ...
    pageElems = DOM.select('img[alt~="Novel"]')
    threads = []
